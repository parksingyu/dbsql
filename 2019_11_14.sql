...
-- FK_EMP_DEPT 활성화
ALTER TABLE emp ENABLE CONSTRAINT fk_emp_dept; -- 제약조건에 위배되는 값이 있어서 활성화 안된다.

--제약조건 위배되는 데이터 (소속부서가 80번인 데이터)가 존재하여 제약조건 활성화 불가
DELETE emp
WHERE empno = 9999;

-- FK_EMP_DEPT 활성화
ALTER TABLE emp ENABLE CONSTRAINT fk_emp_dept; -- 제약조건에 위배되는 값을 삭제했으니 활성화 된다.
commit;

SELECT *
FROM emp;

--현재 계정에 존재하는 테이블 목록 view : USER_TALBES
--현재 계정에 존재하는 제약조건 view : USER_CONSTRAINTS
--현재 계정에 존재하는 제약조건의 컬럼 view : USER_CONS_COLUMS
...
--HR계정에게 v_emp_dept view 조회 권한을 준다.
...
SELECT product.*, ROWID, ROWNUM
FROM product
WHERE ROWID='AAAFOOAAFAAAAFPAAB';
