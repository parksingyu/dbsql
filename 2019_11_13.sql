...
--CHECK 제약 : 컬럼의 값을 정해진 범위, 혹은 값만 돌아오게끔 제약
DROP TABLE emp_test;
CREATE TABLE emp_test(
    empno NUMBER(4),
    ename VARCHAR2(10),
...
--CUSTOMER 테이블을 사용하며 CUSTOMER_TEST 테이블로 생성
--CUSTOMER 테이블의 데이터도 같이 복제(복사)
...
--데이터는 복젲하지 않고 특정 테이블의 컬럼 형식만 거져올 순 없을까?
DROP TABLE customer_test;
CREATE TABLE customer_test AS
SELECT *
FROM customer
WHERE 1=2;  -- 절대로 만족할수 없는 조건을 넣는것이 관건입니다.
...
--데이터 타입을 변경하기 위해서는 컬럼값이 비어 있어야 한다.
...
--3.emp 테이블의 dept컬럼으로 dept 테이블의 deptno컬럼을
